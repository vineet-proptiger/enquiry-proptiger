...
const units = [
  ...
  {
    type: "3 BHK (The Signature)",
    size: "2,275 Sq.ft.",
    price: "Ask For Price",
    oldPrice: null,
    btnText: "Ask For Price",
    features: ["Thoughtful Architecture", "Clubhouse Access", "Optimal Natural Light"],
    isPopular: true,
  },
  {
    type: "3 BHK (The Sky Home)",
    size: "2,455 Sq.ft.",
    price: "Ask For Price",
    oldPrice: null,
    btnText: "Ask For Price",
    features: ["Ultra-Spacious Rooms", "Panoramic Skyline View", "Elite Privacy"],
    isPopular: false,
  },
];

const Pricing = ({ setIsOpen }) => (
  <section id="pricing" className="py-10 sm:py-14 px-4 md:px-8 relative overflow-hidden"
    style={{ background: "var(--color-bg-muted)" }}>
    <div className="absolute top-0 right-0 w-1/3 h-1/3 opacity-5 pointer-events-none"
      style={{ background: `radial-gradient(circle, ${GOLD} 0%, transparent 70%)`, filter: "blur(40px)" }} />
    ...
    <div className="max-w-7xl mx-auto relative z-10">
      <div className="text-center mb-16" data-aos="fade-up">
        <span style={{
          ...
          HYDERABAD — THOUGHTFUL RESIDENCES
        </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-6xl mx-auto">
        {units.map((unit, idx) => (
          <div key={idx} data-aos="fade-up" data-aos-delay={idx * 150}
            ...
            {unit.isPopular && (
              <div className="absolute top-4 right-6 px-3 py-1 rounded-full text-white text-[9px] font-bold tracking-widest uppercase z-30"
              ...
              <div className="space-y-3 mb-8">
                {unit.features.map((feature, fIdx) => (
                  <div key={fIdx} className="flex items-start gap-3">
                    <div className="mt-1 flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center" style={{ background: 'var(--color-teal-bg)' }}>
