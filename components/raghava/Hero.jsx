...
import { heroImages } from '../../lib/raghava/images'
...
export default function Hero({ setIsOpen }) {
  ...
  const specs = [
    { val: '3',                lbl: 'TOWERS'             },
    { val: '52',               lbl: 'FLOORS'             },
    { val: '90,000 sft',       lbl: 'WELLNESS CLUBHOUSE' },
    { val: '1,800–2,455 sft',  lbl: 'HOME SIZES'         },
  ]
    ...
    <>
      ...
      <div className="lg:hidden" style={{ marginTop: '64px', background: '#0d1a24' }}>
        ...
        <div style={{ position: 'relative', width: '100%', height: '56vw', minHeight: '220px', maxHeight: '360px', overflow: 'hidden' }}>
          <div style={{
            ...
            {slides.map((src, idx) => (
              <div key={idx} style={{ flex: '0 0 100%', height: '100%', position: 'relative' }}>
                <Image src={src} alt={`Raghava Halo ${idx + 1}`} fill className="object-cover" priority={idx === 0} sizes="100vw" />
              </div>
            ))}
          </div>
          ...
          <div style={{ position: 'absolute', bottom: '14px', left: '16px', zIndex: 5 }}>
            {/* <p style={{ fontFamily: F_SANS, fontSize: '10px', fontWeight: 600, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--color-teal)', margin: '0 0 4px' }}>
              Thoughtfully Planned High-Rise Living · Hyderabad
            </p> */}
            <h1 style={{ fontFamily: F_JOST, fontSize: 'clamp(24px, 7vw, 36px)', fontWeight: 800, color: '#fff', lineHeight: 1.05, letterSpacing: '-0.02em', margin: 0 }}>
              Raghava <span style={{ color: 'var(--color-teal)' }}>Halo</span>
            </h1>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', background: 'var(--color-bg, #F4F8F9)' }}>
          {specs.map((s, i) => (
            <div key={i} style={{
              ...
              <p style={{ fontFamily: F_JOST, fontSize: s.val.length > 12 ? '16px' : '20px', fontWeight: 700, color: 'var(--color-dark)', lineHeight: 1.1, marginBottom: '3px', whiteSpace: 'nowrap' }}>{s.val}</p>
        ...
        <div style={{ background: '#fff', padding: '14px 16px', borderTop: '1px solid rgba(13,26,36,0.08)' }}>
          ...
          {[
            '5.5 Acres Gated Community in Hyderabad',
            '90,000 SFT Luxury Wellness Clubhouse',
        ...
        <div className="flex flex-row" style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {[
                '3 Towers · 52 Floors · 1,000+ Homes',
                '3 BHK Luxury (1,800–2,455 Sft)',
                '90,000 Sft Wellness Clubhouse',
                '5.5 Acres Gated Community in Hyderabad',
              ].map((line, i) => (
